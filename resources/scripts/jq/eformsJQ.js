	...
	if (postalCode != '' && houseNumber != '') {
		if (postalCode.length == 6) {
			...
			$.ajax({
				dataType : "json",
				type: "GET",
				url: "retrieveAddress.html",
				data: {
					supplierid: '40114112001',
					postal: postalCode + "|" + houseNumber
				}
			})
			.done(function(json) {
				if (json.error == '') {
...
function retrieveHandle(id) {
	...
	function CountWords (this_field, id, maxAllowed) {
		...
		if (word_count >= maxAllowed) {
			bootbox.alert("! Deze tekst mag niet langer zijn dan " + maxAllowed + " woorden.");
		}
